...
import {dummyAccountsData, dummyActivityData, dummyPostsData} from "../assets/assets.tsx";
...
const Dashboard = () => {
    ...
    useEffect(()=>{
        ...
        try {
            const [postsRes,accountsRes,activityRes] = [{data:dummyPostsData},{data:
            dummyAccountsData},{data:dummyActivityData}]
                ...
                {statCards.map((card)=>(
                    ...
                    border-slate-200 rounded-2xl p-5 hover:border-red-200 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="text-xs absolute right-4 top-4 text-red-500
                            flex items-center">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    ...
                    {activities.length === 0 ? (
                        ...
                        <p className="text-slate-500">No activity yet</p>
                            <p className="text-slate-400 text-sm mt-1">Connect account and schedule posts to see events here</p>
                        </div>
                    ):(
                        <div>
                            {activities.map((activity)=>(
                                <div key={activity._id} className="flex items-start gap-4 px-6
                                py-4 hover:bg-slate-50/50 transition-colors">
