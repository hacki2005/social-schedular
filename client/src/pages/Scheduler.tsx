...
import {dummyPostsData, PLATFORMS} from "../assets/assets.tsx";
...
const Scheduler = () => {
    ...
    const tooglePlatform = (id: string) => setSelectedPlatforms((prev) => (prev.includes(id) ?
            ...
            <div className="w-full lg:w-115 shrink-0">
                <div className="bg-white rounded-2xl border border-slate-200 p-6">
                    <div className="flex items-center gap-2 mb-6">
                        <h2 className="text-lg text-slate-700">ComposePost</h2>
                    </div>
                    <form className="space-y-5" onSubmit={handleSchedule}>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {PLATFORMS.map((p) => {
                                    ...
                                    return (
                                        <button key={p.id} type="button" onClick={() => tooglePlatform(p.id)}
                                                className={`flex items-center gap-1.5 p-3 rounded-md border
                                            transition-all duration-150 ${active ?
                                                    "bg-red-50 border-red-300 text-red-500 scale-103" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}>
                        ...
                        <div>
                            ...
                            text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content}
                        ...
                        <div>
                            ...
                            {mediaFile ? (
                                ...
                                border-slate-200 bg-slate-50">
                                    ...
                                    <button type="button" onClick={() => setMediaFile(null)} className="absolute top-2 right-2 size-7 bg-slate-900/60
                                ...
                                <label className="flex items-center justify-center gap-2 p-5 py-10
                                ...
                                hover:border-red-300 hover:bg-red-50/30 transition-all group">
                                    ...
                                    transition-colors">Click to upload image or video</span>
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex gap-1.5 items-center">
                                            {post.platforms.map((pl: string) => {
                                                ...
                                                return meta ?
                                                    <meta.icon key={pl} className="size-3.5 text-slate-400"/> : null
                                            })}
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {post.mediaType && <span
                                                className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-1.5
                                            py-0.5 rounded-md font-semibold capitalize">{post.mediaType}</span>}

                                            <span
                                                className="text-xs text-slate-400">{new Date(post.scheduledFor).toLocaleString()}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            py-0.5 rounded-md font-semibold capitalize">{post.mediaType}</span>}
                                                ...
                                                <span
                                                    className="text-xs text-slate-400">{new Date(post.updatedAt).toLocaleString()}</span>
